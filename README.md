...
- the test can run them in parallel (can reduce the running time).
- The test is more stable (because it's not in order).
### Rule 2. Setting no constant time (sleep) for checking the results of components.
- e.g., waiting for 2 sec to check the result of sorting the array list = bad bad bad practice.
